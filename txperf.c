/*
 * Copyright (c) 2007 //#include "lwip/err.h"
...
int sendData(char character[],int NUM)
{
#if __arm__
	int copy = 3;
#else
	...
	struct tcp_pcb *tpcb = connected_pcb;
	err_t err;
	err = tcp_write(tpcb, send_buf, NUM, copy);
...
start_txperf_application()
{
	...
	IP4_ADDR(&ipaddr,  147, 83,  81, 222);		/* iperf server address */
